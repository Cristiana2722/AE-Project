...
router.put('/:user_id', verifyToken, async (req, res) => {
    try {
        ...
        if (isNaN(id)) {
            ...
            return res.status(400).json({success: false, message: 'User id not valid', data: {}});
        }
        ...
        res.status(200).json({success: false, message: 'User updated successfully', data: updatedUser});
    } catch (error) {
